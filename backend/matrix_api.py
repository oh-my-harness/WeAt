...
async def get_registered_users() -> list[dict]:
    """获取已注册用户列表。本地有缓存时直接返回，否则从用户目录搜索同步。"""
    local = _load_users()
    if local:
        return local
    try:
        return await _sync_users_from_directory()
    except Exception as e:
        logger.warning("Failed to sync users from directory: %s", e)
        return []


async def _sync_users_from_directory() -> list[dict]:
    """通过用户目录搜索获取所有已注册用户。需要 TUWUNEL_SHOW_ALL_LOCAL_USERS_IN_USER_DIRECTORY=true。"""
    token = await _get_admin_token()
    # 搜索 "@" 前缀可以匹配所有本地用户（当 show_all_local_users 启用时）
    data = await _post("/_matrix/client/v3/user_directory/search", token=token, json={
        "search_term": "@",
        "limit": 500,
    })
    ...
    for u in results:
        ...
        if user_id and user_id not in exists:
            users.append({"name": user_id, "deactivated": False})
            exists.add(user_id)
    if users:
        _save_users(users)
    return users
